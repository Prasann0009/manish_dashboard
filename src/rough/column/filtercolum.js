...
// import ColFilterform from "./Filterform"
function DataTable({ columns, data,handleStatus,handleDelete }) {

    const { getTableProps, getTableBodyProps,headerGroups,page,prepareRow,
    nextPage,previousPage,canPreviousPage,canNextPage,state:{pageIndex},pageCount,gotoPage,state,setGlobalFilter,setFilter} = useTable({ columns, data},useFilters ,useGlobalFilter,usePagination);
  const {globalFilter}=state
    return (
      <>
      {/* <Filterform filter={"FIRSTNAME"} setFilter={setFilter}/> */}
  ...
  <ul class="pagination">
    ...
    {/* <li class="page-item "><Link class="page-link" href="#">{(pageCount)}</Link></li> */}
    {/* <li class="page-item"><Link class="page-link" href="#" onClick={()=>(gotoPage((pageIndex)+3))}>{(pageIndex)+3}</Link></li> */}
