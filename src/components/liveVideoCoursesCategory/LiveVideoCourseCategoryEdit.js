...
export default function LiveVideoCourseCategoryEdit () {
  ...
  useEffect(()=>{
      const apiCall =async ()=>{
          let response=await fetch(`http://localhost:8080/api/livevideocoursescategory/details/${id}`,{
            method: "GET",
          });
  ...
  const handelOnSubmit=(e)=>{
    ...
    const apiCall =async ()=>{
      let response=await fetch(`http://localhost:8080/api/liveVideoCoursesCategory/edit/${id}`,{
        method: "POST",
