...
export default function LiveVideoCourseEdit () {
  const navigate = useNavigate();
    let {id}=useParams();
 let [formval,setFormval]=useState({title:'',photos:[],videoCategory:"",details:'',price:'',offerPrice:''});
 ...
 const handelOnChange=(e)=>{
  ...
  const handelOnSubmit=(e)=>{
    ...
    const apiCall =async ()=>{
      let response=await fetch(`http://localhost:8080/api/livevideocourse/edit/${id}`,{
        method: "POST",
        // header:{
  ...
  const handleImgDelete=(imgid)=>{
    console.log("*req")
    const apiCall =async ()=>{
      let response=await fetch(`http://localhost:8080/api/livevideocourse/${id}/deleteimage/${imgid}`,{
        method: "POST",
        ...
        <div className="row align-items-center">
          <div className="col-md-12">
            <ul className="breadcrumb">
              ...
              <li className="breadcrumb-item">
                <a href="javascript:void(0)">Live Video Course  Details
                  </a>
              </li>
