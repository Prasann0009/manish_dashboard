...
function VideoCourseCategorylist() {
   let [tableData,setTableData]=useState([]);
   let [num,setnum]=useState(1)
   useEffect(()=>{
      let apiCall=async ()=>{
        let res = await fetch(`http://localhost:8080/api/videocoursecategory/allcourses`, {
          method: "GET",
...
;
  ...
  const handleDelete=(id)=>{
   // console.log(id);
     ...
     let apiCall=async ()=>{
       let res = await fetch(`http://localhost:8080/api/videocoursecategory/delete/${id}`, {
         method: "POST",
         // headers: {
 ...
 const handleStatus=(id)=>{
   console.log(id);
   let apiCall=async ()=>{
     let res = await fetch(`http://localhost:8080/api/videocoursecategory/active/${id}`, {
       method: "PUT",
      //  headers: {
      //    "Authorization":"eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJfaWQiOiI2NjBlNzAyYWE5MDUxMjg0ZTdmZmFmNDIiLCJwZ29uZSI6MTIzLCJwaW4iOjEyMzQsImlhdCI6MTcxMjIyMjQxOX0.f6tfrIcyO4GdoH6DO849jrzxg4cdkur3-dk042iLeQI"
            ...
            <div className="row align-items-center">
               <div className="col-md-12">
                  <ul className="breadcrumb">
                     ...
                     <li className="breadcrumb-item">
                        <a href="javascript: void(0)">Video Category Management</a>
                     </li>
               ...
               <h5 className='d-flex justify-content-between align-items-center'> <span>Video Course Category List</span>
               <Link to='/video-course-category-add' className='btn btn-primary'>Add Video course Category</Link>
               </h5>
               </div>
                ...
                <tbody>
                  {
                     tableData.map((arr)=>{
                        return (
                           ...
                           <td>
                               <Link to={`/video-course-category-edit/${arr._id}`} className='btn btn-primary btn-sm'>
